...
\begin{enumerate}
    ...
    \item Use logical reasoning and established mathematical principles to derive consequences from this assumption.
...
Contradiction could be used to proof many interesting conclusion in mathematics, here is a classical example that prove the length of a diagonal in a square with side length 1 is irrational number.
\begin{example}
...
\begin{enumerate}
    \item We express $\sqrt{5}$ as a fraction in lowest terms, so we have:
    \[
    \sqrt{5} = \frac{p}{q}
    \]
...
\begin{proof}
Assume, for the sake of contradiction, that there exists a smallest positive rational number \( \frac{p}{q} \), where \( p \) and \( q \) are positive integers with no common factors other than 1, and \( q \neq 0 \).
...
We have know that functions are actually reflection from one set to the other set. We will look into several special mapping.
...
\begin{proof}
    \begin{align*}
LHS & = & \sum _{0\leq k< n} a_{k+1} b_{k} \ -\ \sum _{0\leq k< n} a_{k} b_{k}\\
 & = & \sum _{0\leq k< n} a_{k+1} b_{k} \ -\ \sum _{0\leq k+1< n} a_{k+1} b_{k+1} \ \\
 & = & \sum _{0\leq k< n} a_{k+1} b_{k} \ -\ \sum _{-1\leq k< n-1} a_{k+1} b_{k+1}\\
 & = & \sum _{k=0}^{n-1} a_{k+1} b_{k} \ -\ \sum _{k=-1}^{n-2} a_{k+1} b_{k+1}\\
 & = & a_{n} b_{n-1} -a_{0} b_{0} +\sum _{k=0}^{n-2} a_{k+1} b_{k} \ -\ \sum _{k=0}^{n-2} a_{k+1} b_{k+1} \ \\
 & = & a_{n} b_{n-1} -a_{0} b_{0} +\sum _{0\leq k< n-1} a_{k+1}( b_{k} -b_{k+1})\\
 & = & a_{n} b_{n-1} -a_{0} b_{0} -\sum _{0\leq k< n-1} a_{k+1}( b_{k+1} -b_{k})\\
 & = & a_{n}( b_{n} -b_{n-1}) +a_{n} b_{n-1} -a_{0} b_{0} -\sum _{0\leq k< n} a_{k+1}( b_{k+1} -b_{k})\\
 & = & a_{n}( b_{n} -b_{n-1} +b_{n-1}) \ -a_{0} b_{0} -\sum _{0\leq k< n} a_{k+1}( b_{k+1} -b_{k})\\
 & = & a_{n} b_{n} -a_{0} b_{0} -\sum _{0\leq k< n} a_{k+1}( b_{k+1} -b_{k})\\
 & = & RHS
\end{align*}
\end{proof}
\begin{exercise}
...
%------------------------------------------------
\section{Counting: a perspective from Set Theory}
%------------------------------------------------
...
Let's review this algorithm referring to definition \ref{def:algo}. 
\begin{enumerate}
