...
\end{exercise}
\begin{proof}
    Given \( x \equiv 2 \pmod{8} \) and \( y \equiv 7 \pmod{8} \), we need to prove that 8 divides \( 2(x + y)^{13} + y - 1 \).

First, we find \( x + y \mod 8 \):

\[
x + y \equiv 2 + 7 \equiv 9 \equiv 1 \pmod{8}
\]

Next, we calculate \( (x + y)^{13} \mod 8 \):

\[
(x + y)^{13} \equiv 1^{13} \equiv 1 \pmod{8}
\]

Then, we calculate \( 2(x + y)^{13} \mod 8 \):

\[
2(x + y)^{13} \equiv 2 \cdot 1 \equiv 2 \pmod{8}
\]

Finally, we calculate \( 2(x + y)^{13} + y - 1 \mod 8 \):

\[
2(x + y)^{13} + y - 1 \equiv 2 + 7 - 1 \equiv 8 \equiv 0 \pmod{8}
\]

Since \( 2(x + y)^{13} + y - 1 \equiv 0 \pmod{8} \), it follows that 8 divides \( 2(x + y)^{13} + y - 1 \).
\end{proof}
...
\section{Primes and Greatest Common Divisors}
    ...
    \subsubsection*{Greatest Common Divisors as Linear Combinations}
        ...
        \begin{proof}
            ...
            You can access a more detailed proof \href{https://math.libretexts.org/Bookshelves/Combinatorics_and_Discrete_Mathematics/Elementary_Number_Theory_(Barrus_and_Clark)/01%3A_Chapters/1.09%3A_Bezout's_Lemma}{here}.
        \end{proof}
        \begin{remark}
